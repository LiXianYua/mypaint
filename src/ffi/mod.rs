...
pub unsafe extern "C" fn mypaint_rectangle_expand_to_include_point(
    r: *mut MyPaintRectangle, x: c_int, y: c_int,
) {
    if r.is_null() { return; }
    let rr = &mut *r;
    let mut rect = crate::util::rect::Rect::new(rr.x, rr.y, rr.width, rr.height);
    rect.expand_to_include_point(x, y);
    rr.x = rect.x; rr.y = rect.y; rr.width = rect.width; rr.height = rect.height;
}

#[no_mangle]
pub unsafe extern "C" fn mypaint_rectangle_expand_to_include_rect(
    r: *mut MyPaintRectangle, other: *const MyPaintRectangle,
) {
    if r.is_null() || other.is_null() { return; }
    let rr = &mut *r;
    let oo = &*other;
    let mut rect = crate::util::rect::Rect::new(rr.x, rr.y, rr.width, rr.height);
...
pub struct MyPaintSurface {
    ...
    pub end_atomic: Option<unsafe extern "C" fn(surface: *mut MyPaintSurface, roi: *mut MyPaintRectangles)>,
...
pub unsafe extern "C" fn mypaint_brush_set_smudge_bucket_state(
    self_: *mut MyPaintBrush, bucket_index: c_int,
    r: f32, g: f32, b: f32, a: f32,
    prev_r: f32, prev_g: f32, prev_b: f32, prev_a: f32,
    prev_color_recentness: f32,
) -> c_int {
    if self_.is_null() || bucket_index < 0 { return 0; }
    let ok = handle(self_).set_smudge_bucket_state(
        bucket_index as usize, r, g, b, a, prev_r, prev_g, prev_b, prev_a,
...
pub unsafe extern "C" fn mypaint_brush_get_smudge_bucket_state(
    self_: *const MyPaintBrush, bucket_index: c_int,
    r: *mut f32, g: *mut f32, b: *mut f32, a: *mut f32,
    prev_r: *mut f32, prev_g: *mut f32, prev_b: *mut f32, prev_a: *mut f32,
    ...
    match brush.get_smudge_bucket_state(bucket_index as usize) {
        Some((rv, gv, bv, av, prv, pgv, pbv, pav, pcrv)) => {
            ...
            if !b.is_null() { *b = bv; }
            if !a.is_null() { *a = av; }
            if !prev_r.is_null() { *prev_r = prv; }
            if !prev_g.is_null() { *prev_g = pgv; }
            if !prev_b.is_null() { *prev_b = pbv; }
            if !prev_a.is_null() { *prev_a = pav; }
            if !prev_color_recentness.is_null() { *prev_color_recentness = pcrv; }
            1
        }
        None => 0,
    }
...
#[no_mangle]
pub unsafe extern "C" fn mypaint_surface_end_atomic(self_: *mut MyPaintSurface, roi: *mut MyPaintRectangles) {
    if self_.is_null() { return; }
...
unsafe extern "C" fn fixed_end_atomic(surface: *mut MyPaintSurface, roi: *mut MyPaintRectangles) {
    ...
    if roi.is_null() { return; }
    let r = &mut *roi;
    if r.rectangles.is_null() || r.num_rectangles <= 0 { return; }
    ...
    for (i, bbox) in rects.rects.iter().enumerate() {
        ...
        slot.x = rect.x; slot.y = rect.y; slot.width = rect.width; slot.height = rect.height;
    }
...
pub unsafe extern "C" fn mypaint_tile_request_init(
    data: *mut MyPaintTileRequest, level: c_int,
    tx: c_int, ty: c_int, readonly: c_int,
) {
...
pub unsafe extern "C" fn mypaint_tiled_surface_set_symmetry_state(
    self_: *mut CFixedTiledSurface, active: c_int,
    center_x: f32, center_y: f32, symmetry_angle: f32,
    symmetry_type: c_int, rot_symmetry_lines: c_int,
) {
    if self_.is_null() { return; }
    ...
    (*self_).inner.symmetry_data.set_pending(
        active != 0, center_x, center_y, symmetry_angle, sym_type, rot_symmetry_lines);
}

/// 对应 mypaint_tiled_surface_get_alpha。
#[no_mangle]
pub unsafe extern "C" fn mypaint_tiled_surface_get_alpha(
    self_: *mut CFixedTiledSurface, x: f32, y: f32, radius: f32,
) -> f32 {
    if self_.is_null() { return 0.0; }
    use crate::surface::Surface as _;
...
#[no_mangle]
pub unsafe extern "C" fn mypaint_tiled_surface_begin_atomic(
    self_: *mut CFixedTiledSurface,
) {
    if self_.is_null() { return; }
    use crate::surface::Surface as _;
...
pub unsafe extern "C" fn mypaint_tiled_surface_end_atomic(
    self_: *mut CFixedTiledSurface, roi: *mut MyPaintRectangles,
) {
    if self_.is_null() { return; }
...
fn build_setting_infos() -> &'static [MyPaintBrushSettingInfo] {
    SETTING_INFOS.get_or_init(|| {
        let cnames = SETTING_CNAMES.get_or_init(||
            crate::SETTING_INFO.iter().map(|s|
                std::ffi::CString::new(s.cname).unwrap()).collect());
        let names = SETTING_NAMES.get_or_init(||
            crate::SETTING_INFO.iter().map(|s|
                std::ffi::CString::new(s.name).unwrap()).collect());
        let tips = SETTING_TOOLTIPS.get_or_init(||
            crate::SETTING_INFO.iter().map(|s|
                std::ffi::CString::new(s.tooltip).unwrap()).collect());
        crate::SETTING_INFO.iter().enumerate().map(|(i, s)| MyPaintBrushSettingInfo {
            cname: cnames[i].as_ptr(),
            name: names[i].as_ptr(),
            constant: if s.constant { 1 } else { 0 },
            min: s.min, def: s.def, max: s.max,
...
fn build_input_infos() -> &'static [MyPaintBrushInputInfo] {
    INPUT_INFOS.get_or_init(|| {
        let cnames = INPUT_CNAMES.get_or_init(||
            crate::INPUT_INFO.iter().map(|i|
                std::ffi::CString::new(i.cname).unwrap()).collect());
        let names = INPUT_NAMES.get_or_init(||
            crate::INPUT_INFO.iter().map(|i|
                std::ffi::CString::new(i.name).unwrap()).collect());
        let tips = INPUT_TOOLTIPS.get_or_init(||
            crate::INPUT_INFO.iter().map(|i|
                std::ffi::CString::new(i.tooltip).unwrap()).collect());
        crate::INPUT_INFO.iter().enumerate().map(|(i, info)| MyPaintBrushInputInfo {
            cname: cnames[i].as_ptr(),
...
#[no_mangle]
pub unsafe extern "C" fn mypaint_brush_setting_info(
    id: c_int,
...
#[no_mangle]
pub unsafe extern "C" fn mypaint_brush_input_info(
    id: c_int,
...
pub unsafe extern "C" fn mypaint_mapping_free(self_: *mut Mapping) {
    if !self_.is_null() { drop(Box::from_raw(self_)); }
}
...
#[no_mangle]
pub unsafe extern "C" fn mypaint_mapping_set_n(
    self_: *mut Mapping, input: c_int, n: c_int,
) {
    if self_.is_null() || input < 0 || n < 0 { return; }
    (*self_).set_n(input as usize, n as usize);
}

#[no_mangle]
pub unsafe extern "C" fn mypaint_mapping_get_n(
    self_: *mut Mapping, input: c_int,
) -> c_int {
    if self_.is_null() || input < 0 { return 0; }
    (*self_).get_n(input as usize) as c_int
}

#[no_mangle]
pub unsafe extern "C" fn mypaint_mapping_set_point(
    self_: *mut Mapping, input: c_int, index: c_int, x: f32, y: f32,
) {
    if self_.is_null() || input < 0 || index < 0 { return; }
    (*self_).set_point(input as usize, index as usize, x, y);
}

#[no_mangle]
pub unsafe extern "C" fn mypaint_mapping_get_point(
    self_: *mut Mapping, input: c_int, index: c_int,
    out_x: *mut f32, out_y: *mut f32,
) {
    if self_.is_null() || input < 0 || index < 0 { return; }
...
#[no_mangle]
pub unsafe extern "C" fn mypaint_mapping_calculate(
    self_: *mut Mapping, data: *const f32,
) -> f32 {
    if self_.is_null() || data.is_null() { return 0.0; }
...
pub struct MyPaintTransform {
    pub xx: f32, pub xy: f32, pub x0: f32,
    pub yx: f32, pub yy: f32, pub y0: f32,
}

impl From<Transform> for MyPaintTransform {
    fn from(t: Transform) -> Self {
        Self { xx: t.xx, xy: t.xy, x0: t.x0, yx: t.yx, yy: t.yy, y0: t.y0 }
    }
}
...
pub unsafe extern "C" fn mypaint_transform_rotate_cw(
    t: *const MyPaintTransform, angle: f32,
) -> MyPaintTransform {
    if t.is_null() { return Transform::identity().into(); }
    let base: Transform = MyPaintTransform { xx: (*t).xx, xy: (*t).xy, x0: (*t).x0,
        yx: (*t).yx, yy: (*t).yy, y0: (*t).y0 }.into();
    let c = angle.cos();
    let s = angle.sin();
    let rot = Transform { xx: c, xy: -s, yx: s, yy: c, x0: 0.0, y0: 0.0 };
...
pub unsafe extern "C" fn mypaint_transform_reflect(
    ...
    let c = (2.0 * angle).cos();
    let s = (2.0 * angle).sin();
    let refl = Transform { xx: c, xy: s, yx: s, yy: -c, x0: 0.0, y0: 0.0 };
    base.multiply(&refl).into()
}

#[no_mangle]
pub unsafe extern "C" fn mypaint_transform_translate(
    t: *const MyPaintTransform, dx: f32, dy: f32,
) -> MyPaintTransform {
    if t.is_null() { return Transform::identity().into(); }
    let mut base: Transform = MyPaintTransform { xx: (*t).xx, xy: (*t).xy, x0: (*t).x0,
        yx: (*t).yx, yy: (*t).yy, y0: (*t).y0 }.into();
    base.x0 += dx;
    base.y0 += dy;
    base.into()
...
pub unsafe extern "C" fn mypaint_transform_point(
    t: *const MyPaintTransform, x: f32, y: f32, out_x: *mut f32, out_y: *mut f32,
) {
    if t.is_null() { return; }
    let base: Transform = MyPaintTransform { xx: (*t).xx, xy: (*t).xy, x0: (*t).x0,
        yx: (*t).yx, yy: (*t).yy, y0: (*t).y0 }.into();
    let (nx, ny) = base.transform_point(x, y);
    if !out_x.is_null() { *out_x = nx; }
    if !out_y.is_null() { *out_y = ny; }
}
